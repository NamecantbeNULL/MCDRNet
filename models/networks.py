...
class ResNet(nn.Module):
    def __init__(self, n_feat, scale_resnetfeats, kernel_size, reduction, act, bias, scale_edecoderfeats, num_cab):
        ...
        self.conv_enc1 = nn.Conv2d(n_feat, n_feat + scale_resnetfeats, kernel_size=1, bias=bias)
        self.conv_enc2 = nn.Conv2d(n_feat, n_feat + scale_resnetfeats, kernel_size=1, bias=bias)
        self.conv_enc3 = nn.Conv2d(n_feat, n_feat + scale_resnetfeats, kernel_size=1, bias=bias)

        self.conv_dec1 = nn.Conv2d(n_feat, n_feat + scale_resnetfeats, kernel_size=1, bias=bias)
        self.conv_dec2 = nn.Conv2d(n_feat, n_feat + scale_resnetfeats, kernel_size=1, bias=bias)
        self.conv_dec3 = nn.Conv2d(n_feat, n_feat + scale_resnetfeats, kernel_size=1, bias=bias)
...
class MCDRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=40, scale_edecoderfeats=20, scale_resnetfeats=16, num_cab=8, kernel_size=3,
        ...
        self.stage1_decoder_curv = Decoder(n_feat, kernel_size, reduction, act, bias, scale_edecoderfeats)
        self.stage1_curvconv = conv(n_feat, 1, kernel_size, bias=bias)
        ...
        self.stage2_decoder_curv = Decoder(n_feat, kernel_size, reduction, act, bias, scale_edecoderfeats)
        self.stage2_curvconv = conv(n_feat, 1, kernel_size, bias=bias)
        ...
        self.nam12_curv = NAM(n_feat, kernel_size=1, bias=bias, cha=1)
        self.nam23_img = NAM(n_feat, kernel_size=1, bias=bias, cha=3)
        self.nam23_curv = NAM(n_feat, kernel_size=1, bias=bias, cha=1)
    ...
    def forward(self, x3_img, x3_curv):
        H = x3_img.size(2)
        W = x3_img.size(3)
        ...
        res1_curv = self.stage1_decoder_curv(feat1)
        res1_img = self.stage1_decoder_img(feat1)

        ## Output results of Stage 1
        stage1_curv = self.stage1_curvconv(res1_curv[0]) + x2_curv
        stage1_img = self.stage1_imgconv(res1_img[0]) + x2_img

        ## Apply Non-local Attention Module (NAM)
        x2_namfeats_curv = self.nam12_curv(res1_curv[0], stage1_curv)
        ...
        x2_cat = self.concat12(torch.cat([x2, x2_namfeats_img, x2_namfeats_curv], 1))

        ## Process features through Encoder of Stage 2
        feat2 = self.stage2_encoder(x2_cat, feat1, res1_img)

        ## Pass features through Decoder of Stage 2
        res2_curv = self.stage2_decoder_curv(feat2)
        res2_img = self.stage2_decoder_img(feat2)

        ## Output results of Stage 2
        stage2_curv = self.stage2_curvconv(res2_curv[0]) + x3_curv
        stage2_img = self.stage2_imgconv(res2_img[0]) + x3_img

        ## Apply NAM
        x3_namfeats_curv = self.nam23_curv(res2_curv[0], stage2_curv)
        ...
        x3_cat = self.concat23(torch.cat([x3, x3_namfeats_img, x3_namfeats_curv], 1))

        x3_cat = self.stage3_orsnet(x3_cat, feat2, res2_img)

        stage3_img = self.tail(x3_cat)

        return [stage1_img, stage2_img, stage3_img + x3_img], [stage1_curv, stage2_curv]